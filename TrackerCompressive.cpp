...
CompressiveTracker::CompressiveTracker(void)
{
    ...
    featureNum = 25;	// number of all weaker classifiers, i.e,feature pool
...
void CompressiveTracker::sampleRect(Mat& _image, const Rect& _objectBox, float _rInner, float _rOuter, int _maxSampleNum, vector<Rect>& _sampleBox)
...
void CompressiveTracker::sampleRect(Mat& _image, const Rect& _objectBox, float _srw, vector<Rect>& _sampleBox)
...
{
    Mat _frameGray;
    cvtColor(_frame,_frameGray,CV_RGB2GRAY);

	// compute feature template
	HaarFeature(_objectBox, featureNum);

	// compute sample templates
    sampleRect(_frameGray, _objectBox, rOuterPositive, 0, 1000000, samplePositiveBox);
    sampleRect(_frameGray, _objectBox, rSearchWindow*1.5, rOuterPositive+4.0, 100, sampleNegativeBox);

    integral(_frameGray, imageIntegral, CV_32F);
...
void CompressiveTracker::processFrame(const Mat& _frame, Rect& _objectBox)
{
	...
	// predict
    sampleRect(_frameGray, _objectBox, rSearchWindow,detectBox);
    integral(_frameGray, imageIntegral, CV_32F);
	...
	// update
    sampleRect(_frameGray, _objectBox, rOuterPositive, 0.0, 1000000, samplePositiveBox);
    sampleRect(_frameGray, _objectBox, rSearchWindow*1.5, rOuterPositive+4.0, 100, sampleNegativeBox);
...
bool TrackerCompressiveFace::init (const Mat& _frame, const Face& _face, void* _anyData)
{
    try {
        ...
        {
            cerr << "Error in TrackerCompressiveFace::init() "
        ...
        for (unsigned int part = 0; part != face_parts.size(); ++part )
            trackers[part].init(_frame, *face_parts[part]);

        return 1;
    } catch(...) {
        cerr << "Exception in TrackerCompressiveFace::init()." << endl;
        return 0;
    }
}

bool TrackerCompressiveFace::processFrame (const Mat& _frame, Face& _face)
{
    try {
        ...
        if ( face_parts.size() > 3 )
        {
            cerr << "Error in TrackerCompressiveFace::processFrame() "
                 << "- more than 3 face rectangles." << endl;
            return 0;
        }
        ...
        for (unsigned int part = 0; part != face_parts.size(); ++part )
            trackers[part].processFrame(_frame, *face_parts[part]);

        return 1;
    } catch(...) {
        cerr << "Exception in TrackerCompressiveFace::processFrame()." << endl;
        return 0;
    }
}
